# HOLOPHONE — DOCUMENT MAÎTRE DE PASSATION 3.1.2

**Référence actuelle : Holophone 3.1.2 — Lot 2 / branding VSNT définitif**  
**Android : `versionName "3.1.2"` / `versionCode 44`**  
...
> **Priorité documentaire :** le **Delta courant 3.1.2** ci-dessous prime sur toute règle contradictoire des sections historiques 3.0.0 / 2.9.0. Ces sections restent conservées pour l'architecture métier et l'historique utile.

---


# 0. Delta courant 3.1.2 — à lire en premier

La 3.1.2 conserve le moteur Judy / présence de la 3.1.0 et le boot atomique de la 3.1.1. Elle ajoute uniquement l'identité visuelle VSNT définitive et les ressources launcher Android.

## 0.1 Identité officielle

```text
Produit : Holophone™
Société fictive : VSim NeuroTek (VSNT)
Slogan définitif : Connecting Minds & Bytes
```

Le slogan au pluriel `Minds` remplace l'ancien texte `Connecting Mind & Bytes.` car le visuel définitif et son handoff utilisent tous deux la forme plurielle.

## 0.2 Splash

Les quatre styles restent disponibles : `classic`, `wallace`, `minimal`, `cyber`. Ils partagent désormais la géométrie SVG **VSNT Concept 03 v7** : sphère centrale, tige, deux mains concentriques, arc extérieur et halo final.

La géométrie doit rester celle du handoff ; ne pas redessiner les mains à l'œil. Les couleurs changent par style via `--vsnt-accent`, `--vsnt-soft`, `--vsnt-halo`, `--vsnt-core`.

`startupRestartVsntAnimation()` relance l'animation à chaque affichage. `startupRunVisible()` attend la fin du jingle et au minimum environ 5980 ms afin que le halo final ne soit pas coupé.

Le boot reste strictement :
...
→ verrou retiré
→ animation + jingle
→ interface métier révélée
```

## 0.3 À propos

Le logo complet orange sur fond noir fourni est utilisé via :

```text
www/media/vsnt-logo-full.png
```

Il n'est pas recoloré par les thèmes.

## 0.4 Icône Android

Le symbole blanc fourni sert de source canonique :

```text
assets/branding/vsnt-mark-white-source.png
```

Les ressources `ic_launcher`, `ic_launcher_round` et `ic_launcher_foreground` sont générées pour mdpi à xxxhdpi. Les adaptive icons v26/v33 utilisent un fond transparent `#00000000`.

## 0.5 Sources branding à conserver
...
ARCHITECTURE_BRANDING_3.1.2.md
CHANGEMENTS_3.1.2.md
RELEASE_3.1.2.md
VALIDATION_3.1.2.md
```
...
`RELEASE_3.1.2.cmd` doit être utilisé ; `scripts/release-safe.ps1` pointe sur `scripts/test-stable-v312.js`.
