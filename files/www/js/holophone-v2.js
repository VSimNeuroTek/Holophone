...
(() => {
  ...
  function repeatPrompt(conv){
    if(!conv)return'';
    const last=(conv.msgs||[]).filter(m=>m.w==='judy'&&m.t&&!isBraindanceUiMessage(conv,m)).slice(-14).map(m=>String(m.t).trim()).filter(Boolean);
  ...
  function isBraindanceUiMessage(conv,m){
    ...
    const ts=Number(m.ts)||0;if(!ts||m.w!=='judy')return false;
  ...
  function bootstrapTranscript(conv,pendingText){
    const c=CONTACTS.find(x=>x.id===(conv&&conv.cid)),U=c?uName(c):'V',J=c?(c.first||'Judy'):'Judy';
    ...
    if(list.length)recent='[REPRISE LOCALE RÉCENTE — SOURCE BRUTE, NE PAS RÉINTERPRÉTER]\n'+list.map(m=>{
      const who=m.w==='v'?U:m.w==='judy'?J:'VÉCU';
  ...
  function ownerLabel(c,e){
    ...
    if(e.owner==='character'||e.ty==='elle')return c.first||'Judy';
  ...
  memBlock=function(c,conv,query,budget){
    ...
    const plan=[
      ...
      ['elle','FAITS SUR '+String(c.first||'Judy').toUpperCase()+' — propriétaire : '+String(c.first||'Judy'),4,2],
  ...
  function timelineBlock(c,query){
    ...
    return '\n\nCHRONOLOGIE UTILE\n'+list.map(e=>'· '+dayLabel(e.ts)+' '+hhmm(e.ts)+' — '+
      (e.owner?'[PROPRIÉTAIRE: '+(e.owner==='user'?uName(c):e.owner==='character'?(c.first||'Judy'):'COMMUN')+'] ':'')+
  ...
  push=function(m){const r=basePush(m);try{const c=cur?CONTACTS.find(x=>x.id===cur.cid):null;if(c&&m&&m.w&&m.ts){
    if(m.mus){timelineAdd(c,'music',(m.w==='judy'?'Tu as partagé':'Il t’a envoyé')+' une musique',m.mus.label||'morceau',m.ts);if(m.w==='judy')pEvent(c,'music','Musique partagée',m.mus.label||'morceau',m.ts)}
    else if(m.imgId||m.img){timelineAdd(c,'image',(m.w==='judy'?'Tu as partagé':'Il t’a envoyé')+' une image',m.cap||'image',m.ts);if(m.w==='judy')pEvent(c,'image','Image partagée',m.cap||'image',m.ts)}
    else if(m.yt){timelineAdd(c,'video',(m.w==='judy'?'Tu as partagé':'Il t’a envoyé')+' une vidéo',m.yt.label||'vidéo',m.ts);if(m.w==='judy')pEvent(c,'video','Vidéo partagée',m.yt.label||'vidéo',m.ts)}
    if(m.w==='refuge')pEvent(c,'refuge','Refuge',m.t||'Déplacement dans le Refuge',m.ts);
    if(m.w==='judy'&&m.act)pEvent(c,'interaction','Interaction',m.act.label||m.t||'geste',m.ts);
  ...
  function epEligibleMessages(conv){
    return (conv&&conv.msgs||[]).filter(m=>m&&(m.w==='v'||m.w==='judy')&&!isBraindanceUiMessage(conv,m)&&String(m.t||'').trim());
  }
  function epTags(v){
  ...
  function epNormalizeSource(list){
    ...
    (list||[]).forEach(s=>{
      if(!s||!s.t)return;const x={w:s.w==='judy'?'judy':'v',ts:Number(s.ts)||0,t:String(s.t)};
  ...
  function epLocalTags(msgs){
    ...
    if((msgs||[]).some(m=>m.w==='judy'&&epJudySelfScore(m)>=3))tags.push('judy');
    return epTags(tags);
  }
  function epJudySelfScore(m){
    if(!m||m.w!=='judy')return 0;
  ...
  function epMessageScore(m){
    ...
    if(m.w==='judy')s+=epJudySelfScore(m);
    return s;
  }
  function epCandidateRanges(msgs){
    ...
    /* On choisit des noyaux distincts, pas des fenêtres qui se fusionnent
       ...
       Judy se définit elle-même sont prioritaires. */
  ...
  function epRenderSources(c,ep,maxChars){
    const J=c.first||'Judy',U=uName(c);let out='',used=0;
    for(const s of (ep.sources||[])){
      const who=s.w==='judy'?J:U,line=who+' : « '+String(s.t||'')+' »\n';
  ...
  async function epRecoverCurrentOnce(){
    ...
    /* 2.2.2 : on reconstruit uniquement les épisodes du fil courant.
       ...
       est réversible et ne touche ni aux autres fils ni au moteur Judy. */
  ...
  function sourceMemories(c,q){
    ...
    return'\n\nMÉMOIRE LONGUE — UNIQUEMENT SOURCES CONSERVÉES\n'+take.map(e=>{
      if(e.ty==='socle')return'· [SOCLE JUDY] '+e.t;
  ...
  function crossThreadRawRecall(c,conv,q){
    ...
    CONVS.filter(v=>v.cid===c.id&&v.id!==conv.id&&!isMindArchived(c,v)).forEach(v=>(v.msgs||[]).forEach(m=>{
      if(!m||!m.t||(m.w!=='v'&&m.w!=='judy')||isBraindanceUiMessage(v,m))return;const T=tokens(m.t);let s=0;words.forEach(w=>{if(T.has(w))s++});if(s>=2)hits.push({s,m});
    }));
    hits.sort((a,b)=>b.s-a.s||(b.m.ts||0)-(a.m.ts||0));const take=hits.slice(0,2);if(!take.length)return'';
    return'\n\nÉCHOS D’ANCIENS FILS — CITATIONS ORIGINALES\n'+take.map(x=>'· '+dayLabel(x.m.ts)+' — '+(x.m.w==='v'?uName(c):(c.first||'Judy'))+' : « '+clipText(x.m.t,220)+' »').join('\n')+
  ...
  function tinyMedia(c,q){
    ...
    if(SPOT&&SPOT.coop&&SPOT.coop.id)s+=' Vous avez une playlist commune « '+clipText(SPOT.coop.name||'V × Judy — Holophone',80)+' ». Si tu proposes un morceau que TU veux vraiment y conserver, mets coop=true avec mus. Ne le fais pas pour chaque morceau.';
  ...
  function currentState(c,conv){
    ...
    if(st==='work')s+=' Selon ses horaires, Judy est normalement au travail'+(c.job?' ('+c.job+')':'')+'.';else if(st==='sleep')s+=' Selon ses horaires, Judy est normalement en période de sommeil.';
    return s;
  }
  function outputRules(c){
    return'\n\nSTYLE DE CE TOUR\nRéponds en français comme '+(c.first||'Judy')+' qui écrit sur téléphone : naturel, vivant, pas administratif. 1 à 3 bulles en général. '+
      'Ne récite pas la mémoire et n’essaie pas de caser un souvenir à chaque réponse. Si tu ne sais pas, tu peux le dire. '+
      'Ne confonds jamais la vie de '+uName(c)+' avec celle de '+(c.first||'Judy')+'. Les citations gardent toujours leur auteur. '+
  ...
  sysPrompt=function(c,conv){
    ...
    const U=uName(c),J=c.first||'Judy',q=contextQuery(conv);let s='IDENTITÉ\nTu es '+J+(c.last?' '+c.last:'')+(c.age?' · '+c.age+' ans':'')+'. Tu échanges avec '+uDesc(c)+'. '+
  ...
  recallBits=function(c,conv,q){
    ...
    return hits.slice(0,2).map(h=>'· '+dayLabel(h.m.ts)+' — '+(h.m.w==='v'?uName(c):(c.first||'Judy'))+
  ...
  deckBar=function(){
    ...
    if(cur&&isMindArchived(c,cur)){
      ...
      n.textContent='Archive d’avant réinitialisation. Tu peux relire ce fil, mais Judy n’y a plus accès et on ne peut plus écrire dedans.';
  ...
  function injectMindReset(){
    ...
    n.textContent='Conserve identité, personnalité, socle, avatars, galerie, jouets et réglages. Efface souvenirs appris, goûts, chronologie, activité et humeur acquise. Les anciennes conversations restent consultables mais deviennent invisibles pour Judy.';
    const b=document.createElement('button');b.className='mini v2-danger';b.textContent='Réinitialiser l’esprit de '+(c.first||'Judy');
    b.onclick=()=>askConfirm('Réinitialiser l’esprit de '+(c.first||'Judy')+' ?',
      ...
      'Continuer',()=>askConfirm('Dernière confirmation',
        'Cette opération vide réellement sa tête acquise. Les conversations ne sont pas supprimées, mais Judy ne pourra plus les relire ni en tirer de nouveaux souvenirs.',
  ...
  function injectMemorySourceUi(){
    ...
    latest.forEach(ep=>{
      ...
      src.textContent=s0?((s0.w==='judy'?(c.first||'Judy'):uName(c))+' : « '+clipText(s0.t,120)+' »'):'';
  ...
  refreshSys=function(){baseRefreshSysPresence();try{const c=C();if(c){const p=presenceUpdate(c,false,cur&&cur.cid===c.id?cur:null);if(q('#presenceMenuLabel'))q('#presenceMenuLabel').textContent='Vie de '+(c.first||'Judy');if(q('#presenceState')&&p)q('#presenceState').textContent=((c.life&&c.life.name)||'présente')+' · '+p.focus}}catch(e){}};
  ...
  function presenceRender(){
    ...
    q('#presenceTitle').textContent='Vie de '+(c.first||'Judy');q('#presenceSub').textContent=p.focus||'présence intérieure';
    B.innerHTML='';
