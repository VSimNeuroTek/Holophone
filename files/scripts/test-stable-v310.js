...
test('Version web 3.1.0',()=>{contains(html,"const APPV='3.1.0'");contains(v2,"version: '3.1.0'");return'APPV et V2'});
...
test('Schéma 3100 et migrations',()=>{contains(stable,'const SCHEMA_VERSION = 3100');contains(stable,'baseline-stable-3.0.0');contains(stable,'judy-presence-3.1.0');return'schéma 3100'});
...
test('PIN avant splash',()=>{const lock=html.indexOf('if(PIN.on&&PIN.code)lockNow()'),splash=html.indexOf("maybeRunStartupSplash(120)",lock);assert(lock>0&&splash>lock,'ordre PIN/splash non vérifié');return'ordre boot sécurisé'});
test('Déverrouillage biométrique conserve le splash',()=>{contains(v2,'maybeRunStartupSplash(120)');return'biométrie alignée sur le PIN'});
...
test('Fichiers stabilité présents',()=>{['www/js/holophone-stability.js','TESTER_STABLE_3.1.0.cmd','RELEASE.cmd'].forEach(p=>assert(exists(p),'absent: '+p));return'outillage présent'});

test('Présence Judy persistante',()=>{contains(v2,'const PRESENCE_VERSION=1');contains(v2,'function presenceUpdate');contains(v2,'function presenceBlock');contains(v2,'window.holoPresenceInitiative');return'présence + initiative'});
test('Écran Vie du personnage',()=>{contains(html,'id="screenPresence"');contains(html,'id="goPresence"');contains(v2,"textContent='Vie de '+(c.first||'Judy')");return'UI dynamique'});
test('Santé accepte les messages Refuge',()=>{contains(stable,"['judy','v','sys','refuge']");return'faux positif supprimé'});
...
console.log('');
console.log('HOLOPHONE 3.1.0 — RECETTE STATIQUE');
console.log('===================================');
