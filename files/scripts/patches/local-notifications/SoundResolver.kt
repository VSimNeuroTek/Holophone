...
internal object SoundResolver {
    ...
    fun resolveUri(context: Context, sound: String?): Uri? {
        ...
        return resolveFromPrivateFiles(context, sound) ?: resolveFromAssets(context, sound)
    }

    private fun resolveFromPrivateFiles(context: Context, sound: String): Uri? {
        val fileName = if (sound.contains("/")) sound.substring(sound.lastIndexOf('/') + 1) else sound
        val file = File(File(context.filesDir, COPY_DIR), fileName)
    ...
    private fun resolveFromAssets(context: Context, sound: String): Uri? {
        ...
        val cleanSound = sound.trimStart('/')
        if (cleanSound.contains('/')) {
            for (folder in ASSET_FOLDERS) {
                val candidate = "$folder/$cleanSound"
                val exists = try { assets.open(candidate).use { true } } catch (e: Exception) { false }
                if (exists) {
