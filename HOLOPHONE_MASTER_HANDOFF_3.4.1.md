# HOLOPHONE MASTER HANDOFF 3.4.0

Version cible : **3.4.0** · versionCode **54** · schemaVersion **3200**.

## Evolution 3.4.0 — Ergo + Prompt Diet

Voir `ARCHITECTURE_ERGO_PROMPT_DIET_3.4.0.md` et `CHANGEMENTS_3.4.0.md`.

Points structurants : Gemini Interaction redevient le moteur central ; vie locale utilisée surtout comme observateur/timing ; contexte Refuge/actions/activité à la demande ; reset unique des Interaction IDs ; Spotify et Refuge refondus ; tailles de textarea mémorisées ; panneaux repliés par défaut.

---

## Historique / handoff précédent

# HOLOPHONE MASTER HANDOFF 3.3.1

Version cible : **3.3.1** — Android `versionCode 52` — schéma structuré `3200`.

## Delta 3.3.1 — hotfix prioritaire

La 3.3.1 est cumulative par rapport à la 3.3.0 et corrige quatre régressions observées sur appareil :

- Spotify : `c` hors portée lors du clic sur un résultat de recherche ;
- Refuge : `c` hors portée dans `drawMsg()` pour une proposition ;
- Refuge : événement automatique pouvant s'intercaler entre une question utilisateur et la réponse IA ;
- Continuité 3.2 : activité trop souvent convertie en narration spontanée.

Règle ajoutée : `m` est le canal des messages tapés. La vie interne est un contexte silencieux ; les gestes structurés passent par `act` quand pertinent.

Aucune migration de données. Toute l'infrastructure i18n/confidentialité 3.3.0 est conservée.

---

## Référence précédente (3.3.0)

## Handoff 3.3.0 intégré
...
# ADDENDUM 3.4.1 — Release technique sauvegarde & réglages

Version 3.4.1 / versionCode 55 / schema 3200.

## Sauvegarde globale v13
- export : `Documents/Holophone/saves/holophone-global-<timestamp>.json` ;
- structures métier complètes + snapshot brut localStorage/Store + IndexedDB `holophone-media/files` ;
- médias inclus systématiquement dans la sauvegarde fichier ;
- clés API optionnelles, désactivées par défaut ;
- import v13 restaure les structures, le stockage binaire, les préférences puis recharge l app ;
- compatibilité v12 maintenue.

## Correctifs techniques
- `toyList(c)` manquant réparé ; `c.toys` est la source canonique ;
- bouton `Vérifier le stockage` séparé visuellement du diagnostic ;
- audit du hub Réglages et des boutons de maintenance : aucun bouton principal orphelin détecté ;
- recette `scripts/test-stable-v341.js` : 49 contrôles statiques.

## Validation
- recette 49/49 avant `cap sync android` ;
- `cap sync android` réussi, 5 plugins Capacitor détectés ;
- recette 49/49 après sync ;
- Gradle non compilé dans l environnement de génération : `services.gradle.org` inaccessible (`UnknownHostException`) avant compilation.
