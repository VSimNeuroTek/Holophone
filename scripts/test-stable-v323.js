...
test('Version web 3.2.3',()=>{contains(html,"const APPV='3.2.3'");contains(v2,"version: '3.2.3'");contains(stable,"const STABLE_VERSION = '3.2.3'");return'APPV, V2 et stabilité'});
...
test('Fichiers stabilité présents',()=>{['www/js/holophone-stability.js','TESTER_STABLE_3.2.3.cmd','RELEASE_3.2.3.cmd','RELEASE.cmd','scripts/patches/android/MainActivity.java'].forEach(p=>assert(exists(p),'absent: '+p));return'outillage présent'});
...
test('En-tête À propos flottant centré 3 cm',()=>{
  contains(html,'class="aboutfloatinghead"');contains(html,'class="aboutfloatlogo"');contains(html,'width:114px;height:114px');contains(html,'class="aboutheadtitle" id="abTitle"');contains(html,'border:0;outline:0;box-shadow:none');
  assert(!html.includes('class="aboutfloatdock"'),'ancienne disposition latérale encore rendue');assert(!html.includes('id="abSubtitle"'),'ancien sous-titre du header encore rendu');return'logo centré ~3 cm + titre dessous dans zone fixe';
});
test('Drapeaux À propos agrandis et centrés',()=>{contains(html,'width:48px;height:34px');contains(html,'font-size:24px');contains(html,'gap:12px');return'sélecteur FR/EN plus lisible';});
...
test('Audit global sans prénom historique dans le code',()=>{
  ...
  roots.forEach(walk);['02_PREPARER_ANDROID_STUDIO.cmd','RELEASE.cmd','RELEASE_3.2.3.cmd','TESTER_STABLE_3.2.3.cmd','package.json','package-lock.json','release-state.json'].forEach(walk);
...
console.log('');
console.log('HOLOPHONE 3.2.3 — RECETTE STATIQUE');
console.log('===================================');
