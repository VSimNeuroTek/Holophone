...
test('Version web 3.1.4',()=>{contains(html,"const APPV='3.1.4'");contains(v2,"version: '3.1.4'");contains(stable,"const STABLE_VERSION = '3.1.4'");return'APPV, V2 et stabilité'});
...
test('Schéma 3140 et migration persona',()=>{contains(stable,'const SCHEMA_VERSION = 3140');contains(stable,'baseline-stable-3.0.0');contains(stable,'character-presence-3.1.0');contains(stable,'persona-neutral-3.1.4');return'schéma 3140'});
...
test('Fichiers stabilité présents',()=>{['www/js/holophone-stability.js','TESTER_STABLE_3.1.4.cmd','RELEASE_3.1.4.cmd','RELEASE.cmd','scripts/patches/android/MainActivity.java'].forEach(p=>assert(exists(p),'absent: '+p));return'outillage présent'});
...
test('Audit global sans prénom historique dans le code',()=>{
  ...
  const roots=['www','scripts','android/app/src/main'];const bad=[];
  ...
  roots.forEach(walk);['02_PREPARER_ANDROID_STUDIO.cmd','RELEASE.cmd','RELEASE_3.1.4.cmd','TESTER_STABLE_3.1.4.cmd','package.json','package-lock.json','release-state.json'].forEach(walk);
...
console.log('');
console.log('HOLOPHONE 3.1.4 — RECETTE STATIQUE');
console.log('===================================');
