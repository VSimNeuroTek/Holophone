...
test('Version 3.4.1 cohérente',()=>{has(html,"const APPV='3.4.1'");has(v2,"version: '3.4.1'");has(stable,"const STABLE_VERSION = '3.4.1'");return'web + V2 + stabilité'});
...
test('Release 3.4.1 code 55',()=>{has(release,'VERSION_NAME=3.4.1');has(release,'VERSION_CODE=55');has(read('RELEASE_3.4.1.cmd'),'3.4.1 55');return'OK'});
test('Release utilise recette 3.4.1',()=>{has(safe,'test-stable-v341.js');return'OK'});
...
test('Sauvegarde globale v13',()=>{has(html,"app:'holophone-backup',v:13");has(html,'raw:{localStorage:');has(html,'indexedDB:withImg?await backupIdbExport():[]');return'format exhaustif'});
test('Sauvegarde globale vers Documents/Holophone/saves',()=>{has(html,"saveFile('holophone-global-");has(html,",txt,'saves')");has(html,"EXPORT_DIR+'/saves/.essai'");return'chemin demandé'});
test('Sauvegarde fichier inclut toujours les médias',()=>{has(html,"buildBackup(true,$('#svKeys').classList.contains('on'))");has(html,'Médias inclus dans la sauvegarde globale');return'images + audio + IDB'});
test('Snapshot préférences locales',()=>{has(html,'function backupLocalStorageExport(withKeys)');has(html,'function backupStoreExport(withKeys)');return'UI + paramètres périphériques'});
test('Snapshot IndexedDB complet',()=>{has(html,'async function backupIdbExport()');has(html,"kind:'blob'");return'médias binaires'});
test('Restauration exhaustive v13',()=>{has(html,'async function backupIdbImport(entries)');has(html,'backupStoreImport(o.raw.store');has(html,'backupLocalStorageImport(o.raw.localStorage');return'raw restore'});
test('Restauration redémarre l app',()=>{has(html,'setTimeout(()=>location.reload(),900)');return'état runtime rechargé'});
test('Compatibilité sauvegardes v12 conservée',()=>{has(html,"o.app==='holophone-backup'");has(html,'Array.isArray(o.contacts)');return'anciens exports acceptés'});
test('toyList mort réparé',()=>{has(html,'function toyList(c){if(!c)return[];if(!Array.isArray(c.toys))c.toys=[];return c.toys}');return'c.toys canonique'});
test('Vérifier stockage ne chevauche plus analyse',()=>{has(html,'id="svDiag" style="min-height:72px;position:relative;z-index:1;padding-bottom:6px"');has(html,'id="svCheck" style="position:relative;z-index:1;width:100%;margin-top:10px"');return'layout séparé'});
test('Hub réglages : entrées principales câblées',()=>{const ids=['goIdent','goPersona','goMood','goPresence','goMem','goAff','goDeep','goAuto','goAvatars','goGal','goBD','goMe','goBrain','goVoice','goSpot','goSearch','goActions','goBDCfg','goBDElems','goBDReplay','goTransfer','goSave','goStartup','goAppearance','goLanguage','goTheme','goLock','goHood','goAbout'];const all=html+'\n'+v2+'\n'+ux+'\n'+i18n;ids.forEach(id=>assert(all.includes("$('#"+id+"')")||all.includes("q('#"+id+"')"),id+' sans gestionnaire'));return ids.length+' entrées'});

const failed=results.filter(r=>!r.ok);
console.log('\nHOLOPHONE 3.4.1 — RECETTE STATIQUE\n===================================');
