...
test('Version web 3.2.1',()=>{contains(html,"const APPV='3.2.1'");contains(v2,"version: '3.2.1'");contains(stable,"const STABLE_VERSION = '3.2.1'");return'APPV, V2 et stabilité'});
...
test('Fichiers stabilité présents',()=>{['www/js/holophone-stability.js','TESTER_STABLE_3.2.1.cmd','RELEASE_3.2.1.cmd','RELEASE.cmd','scripts/patches/android/MainActivity.java'].forEach(p=>assert(exists(p),'absent: '+p));return'outillage présent'});
...
test('Audit global sans prénom historique dans le code',()=>{
  ...
  roots.forEach(walk);['02_PREPARER_ANDROID_STUDIO.cmd','RELEASE.cmd','RELEASE_3.2.1.cmd','TESTER_STABLE_3.2.1.cmd','package.json','package-lock.json','release-state.json'].forEach(walk);
...
console.log('');
console.log('HOLOPHONE 3.2.1 — RECETTE STATIQUE');
console.log('===================================');
